(function(){
 ...
 const blob=s=>normalize([s.title,s.artist,s.style,s.key,s.content,s.notes,...(s.tags||[])].filter(Boolean).join(' '));
 const create=p=>{const n=new Date().toISOString();return{id:p.id||`song_${Date.now()}`,title:String(p.title||'').trim()||'Sem título',artist:String(p.artist||'').trim()||'Sem artista',style:String(p.style||'').trim(),key:String(p.key||'C').trim(),bpm:String(p.bpm||'72').trim(),content:String(p.content||'').trim(),notes:String(p.notes||'').trim(),tags:Array.isArray(p.tags)?p.tags:String(p.tags||'').split(',').map(x=>x.trim()).filter(Boolean),capo:Math.max(0,Math.min(12,Number(p.capo||0))),createdAt:p.createdAt||n,updatedAt:n,source:'local'}};
 const api={
  ...
  saveDraft(p){const c=this.getDraft()||{},d={id:p.id||c.id||`draft_${Date.now()}`,title:String(p.title||'').trim(),artist:String(p.artist||'').trim(),style:String(p.style||'').trim(),key:String(p.key||'C').trim(),bpm:String(p.bpm||'72').trim(),content:String(p.content||'').trim(),notes:String(p.notes||'').trim(),tags:String(p.tags||'').trim(),capo:Math.max(0,Math.min(12,Number(p.capo||0))),updatedAt:new Date().toISOString()};set(DRAFT_KEY,d);return d},getDraft(){return get(DRAFT_KEY,null)},clearDraft(){window.StageMusicSafeStorage?.remove(DRAFT_KEY)||localStorage.removeItem(DRAFT_KEY)},
