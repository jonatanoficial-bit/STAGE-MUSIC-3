(function(){
  ...
  async function buildReport(){
    ...
    const globalSongs=readArray('stage_music_global_songs_v1');
    ...
    const setup=[
      ...
      check('songs','Biblioteca privada',songs.length?'ok':'warn',songs.length?`${songs.length} cifra${songs.length===1?'':'s'} privada${songs.length===1?'':'s'} disponível${songs.length===1?'':'is'}.`:'Crie ou baixe sua primeira cifra.',5),
      check('global-catalog','Catálogo global',globalSongs.length?'ok':'info',globalSongs.length?`${globalSongs.length} cifra${globalSongs.length===1?'':'s'} global${globalSongs.length===1?'':'is'} em cache.`:'O catálogo global será carregado após o login e a primeira atualização.',4),
