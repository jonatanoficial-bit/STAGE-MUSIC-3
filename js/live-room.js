...
function startRemoteSubscription(){
 ...
 remoteUnsubscribe=window.StageMusicFirebaseLive.subscribe(current.code,(remote)=>{if(!remote)return;window.StageMusicLiveStability?.markRemoteRoom?.(current.code);remoteApplying=true;rooms[current.code]=normalizeRoom(remote);write(ROOMS_KEY,rooms);activateRoomSetlist(rooms[current.code]);remoteApplying=false;render();channel?.postMessage({type:'rooms-updated',at:now(),remote:true})},error=>{console.warn('Escuta da Sala Live online:',error?.message||error);window.StageMusicLiveStability?.queueRoom?.(room(),'retry')});
}
function saveRooms(){
 write(ROOMS_KEY,rooms);channel?.postMessage({type:'rooms-updated',at:now()});render();const r=room();
 if(r&&!remoteApplying&&window.StageMusicFirebaseLive?.isReady?.()&&window.StageMusicAuth?.getState?.().mode==='online')return (window.StageMusicLiveStability?.safePutRoom?.(r,{source:'sala-live'})||window.StageMusicFirebaseLive.putRoom(r)).catch(error=>{console.warn('Sala Live online:',error?.message||error);throw error});
 return Promise.resolve();
}
function startHeartbeat(){clearInterval(heartbeat);if(!current)return;heartbeat=setInterval(()=>{const r=room(),m=activeMember();if(!r||!m)return;m.lastSeenAt=now();r.updatedAt=now();write(ROOMS_KEY,rooms);channel?.postMessage({type:'heartbeat',code:r.code});window.StageMusicLiveStability?.renderRoomHealth?.()},8000)}
...
async function joinRoom(options={}){
 ...
 const mid=memberId('member'),profile=getProfile(),name=$('room-display-name').value.trim()||profile.name||'Músico',instrument=$('room-instrument').value||profile.instrument||'Outro',viewMode=selectedViewMode('member');persistViewChoice(c,viewMode);saveProfile();
...
function move(step){const r=room(),list=currentSetlist(r);if(!r||!list)return;const next=Math.max(0,Math.min((list.songs?.length||1)-1,r.index+step));if(next===r.index)return toast(step<0?'Início do repertório':'Fim do repertório');if(window.StageMusicLiveStability?.isDuplicateAction?.(`room-move-${next}`,850))return;r.index=next;r.command='';r.commandAt='';r.flow=defaultFlow();r.updatedAt=now();activateRoomSetlist(r);saveRooms().catch(()=>{});window.dispatchEvent(new CustomEvent('stage-room-position',{detail:{index:next,room:r}}))}
function sendCommand(text){const r=room(),message=String(text||'').trim();if(!r||!message||current?.role!=='director')return;if(window.StageMusicLiveStability?.isDuplicateAction?.(`command-${message}`,900))return toast('Comando já enviado.');r.command=message;r.commandAt=now();r.commandType='message';r.messages=[{id:`msg_${Date.now()}`,text:message,at:r.commandAt,author:activeMember()?.name||'Direção musical'},...(r.messages||[])].slice(0,20);r.updatedAt=now();saveRooms().catch(()=>{});if($('director-message'))$('director-message').value='';toast('Aviso destacado enviado aos músicos.')}
...
function bind(){
 ...
 $('room-display-name').addEventListener('input',()=>{$('join-profile-name').textContent=$('room-display-name').value.trim()||'Músico'});const updateJoinSummary=()=>{const mode=selectedViewMode(current?.role||'member');$('join-profile-instrument').textContent=`${$('room-instrument').value} • ${viewModeLabel(mode)}`;if($('join-room-btn'))$('join-room-btn').textContent=mode==='lyrics'?'Entrar e abrir letra':'Entrar e abrir cifra';persistViewChoice($('join-room-code')?.value,mode);saveProfile()};$('room-instrument').addEventListener('change',updateJoinSummary);$('room-view-mode')?.addEventListener('change',updateJoinSummary);setTimeout(updateJoinSummary,0);
 ...
 $('join-room-code').addEventListener('input',event=>{const caret=event.target.selectionStart;event.target.value=normalizeCode(event.target.value);event.target.setSelectionRange?.(Math.min(caret+1,event.target.value.length),Math.min(caret+1,event.target.value.length));persistViewChoice(event.target.value,selectedViewMode(current?.role||'member'))});window.StageMusicQRScanner?.attach?.({buttonId:'room-scan-qr',inputId:'join-room-code',toast,label:'Aponte para o QR Code da Sala Live'});
...
async function handleInviteFromUrl(){const params=new URLSearchParams(location.search),raw=params.get('room')||params.get('code');if(!raw){document.documentElement.classList.remove('room-invite-pending');return;}document.body.classList.add('room-invite-mode');const c=normalizeCode(raw);$('join-room-code').value=c;$('join-invite-hint').hidden=false;const shouldAuto=params.get('auto')==='1'||params.has('view')||params.has('role')||params.has('instrument');if(!shouldAuto){document.documentElement.classList.remove('room-invite-pending');toast('Convite carregado. Escolha cantar ou tocar e toque em entrar.');setTimeout(()=>$('join-room-btn')?.focus(),180);return}const auth=window.StageMusicAuth?.getState?.()||{};const localRoom=rooms[c];if(window.StageMusicFirebaseLive?.isReady?.()&&auth.mode!=='online'&&!localRoom){showGuestLogin(c);document.documentElement.classList.remove('room-invite-pending');toast('Entre com Google para abrir esta sala em tempo real.');return}setTimeout(()=>joinRoom({navigate:true,automatic:true}),550)}
