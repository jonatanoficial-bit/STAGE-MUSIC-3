...
if(info.phase<39)errors.push('BUILD-INFO abaixo da Fase 39.');
const live=fs.readFileSync('modo-live.html','utf8');
for(const token of ['live-view-toggle','js/live-choice.js','Aguardando'])if(!live.includes(token))errors.push(`Modo Live sem ${token}`);
...
for(const token of ['room-scan-qr','js/qr-scanner.js','Ler QR'])if(!room.includes(token))errors.push(`Sala Live sem ${token}`);
...
for(const token of ['stage_music_reader_choice_','Letra','Cifra','setDisplayMode'])if(!choice.includes(token))errors.push(`live-choice.js sem ${token}`);
...
for(const token of ['stage-music-v4-','./js/qr-scanner.js'])if(!sw.includes(token))errors.push(`Service Worker sem ${token}`);
